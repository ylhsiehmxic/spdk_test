...
/* --------------------------------------------------------------------- */
/*
 * For single-IO latency, we trace a per-request "ublk IO object" keyed by (qid, tag).
 * This object is distinct from SPDK bdev IO (OBJECT_BDEV_IO, 'i'), because ublk.c
 * does not have a stable spdk_bdev_io* pointer to relate to.
 *
 * If you later extend this module to capture the spdk_bdev_io* used in bdev tracing,
 * you can add spdk_trace_tpoint_register_relation() entries to OBJECT_BDEV_IO.
 */
#define TRACE_GROUP_UBLK            0x90  /* NOTE: adjust if this conflicts with your tree */
#define TRACE_UBLK_REQ_READY        SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x0)
#define TRACE_UBLK_BUF_WAIT_BEGIN   SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x1)
#define TRACE_UBLK_BUF_WAIT_DONE    SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x2)
#define TRACE_UBLK_BDEV_SUBMIT      SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x3)
#define TRACE_UBLK_BDEV_DONE        SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x4)
#define TRACE_UBLK_COMMIT_PREP      SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x5)
#define TRACE_UBLK_COMMIT_SUBMIT    SPDK_TPOINT_ID(TRACE_GROUP_UBLK, 0x6)

#define OWNER_TYPE_UBLK             0x90
#define OBJECT_UBLK_IO              0x90

static inline uint64_t
ublk_trace_oid(const struct ublk_queue *q, const struct ublk_io *io)
{
	(void)q;
	/* Use the per-tag IO context pointer as the object id so that:
	 *  1) All UBLK_* events for the same tag share the same 'uXXXX' id, and
	 *  2) We can relate BDEV_IO_START/DONE ('iXXXX') to this UBLK object via the
	 *     BDEV tracepoint 'ctx' argument (which is our cb_arg = io pointer).
	 *
	 * Note: tags are reused, so this object id will be reused over time.
	 */
	return (uint64_t)(uintptr_t)io;
}

static void
ublk_trace_init(void)
{
	struct spdk_trace_tpoint_opts opts[] = {
		{
			"UBLK_REQ_READY", TRACE_UBLK_REQ_READY,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 1,
			{
				{ "qid",   SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",   SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "op",    SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "lba",   SPDK_TRACE_ARG_TYPE_INT, 8 },
				{ "secs",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "cmdop", SPDK_TRACE_ARG_TYPE_INT, 4 },
			}
		},
		{
			"UBLK_BUF_WAIT_BEGIN", TRACE_UBLK_BUF_WAIT_BEGIN,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 0,
			{
				{ "qid",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "size", SPDK_TRACE_ARG_TYPE_INT, 8 },
			}
		},
		{
			"UBLK_BUF_WAIT_DONE", TRACE_UBLK_BUF_WAIT_DONE,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 0,
			{
				{ "qid",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",  SPDK_TRACE_ARG_TYPE_INT, 4 },
		...
		{
			"UBLK_BDEV_SUBMIT", TRACE_UBLK_BDEV_SUBMIT,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 1,
			{
				{ "qid",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "op",   SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "lba",  SPDK_TRACE_ARG_TYPE_INT, 8 },
				{ "blks", SPDK_TRACE_ARG_TYPE_INT, 8 },
			}
		},
		{
			"UBLK_BDEV_DONE", TRACE_UBLK_BDEV_DONE,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 0,
			{
				{ "qid",    SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",    SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "status", SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "cmdop",  SPDK_TRACE_ARG_TYPE_INT, 4 },
			}
		},
		{
			"UBLK_COMMIT_PREP", TRACE_UBLK_COMMIT_PREP,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 0,
			{
				{ "qid",    SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",    SPDK_TRACE_ARG_TYPE_INT, 4 },
		...
		{
			"UBLK_COMMIT_SUBMIT", TRACE_UBLK_COMMIT_SUBMIT,
			OWNER_TYPE_UBLK, OBJECT_UBLK_IO, 0,
			{
				{ "qid",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "tag",  SPDK_TRACE_ARG_TYPE_INT, 4 },
				{ "cnt",  SPDK_TRACE_ARG_TYPE_INT, 4 },
			}
		},
	};

	spdk_trace_register_owner_type(OWNER_TYPE_UBLK, 'u');
	spdk_trace_register_object(OBJECT_UBLK_IO, 'u');
	spdk_trace_register_description_ext(opts, SPDK_COUNTOF(opts));

	/* Relate BDEV_IO_START/DONE ('iXXXX') to our UBLK object ('uXXXX') via the
	 * BDEV tracepoint 'ctx' argument (arg index 1). In our bdev submits, cb_arg
	 * is (struct ublk_io *) which matches our ublk_trace_oid() object id.
	 */
	spdk_trace_tpoint_register_relation(TRACE_BDEV_IO_START, OBJECT_UBLK_IO, 1);
	spdk_trace_tpoint_register_relation(TRACE_BDEV_IO_DONE, OBJECT_UBLK_IO, 0);
}

SPDK_TRACE_REGISTER_FN(ublk_trace_init, "ublk", TRACE_GROUP_UBLK)
...
ublk_io_done(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	...
	spdk_trace_record(TRACE_UBLK_BDEV_DONE, OWNER_TYPE_UBLK, 0,
			  ublk_trace_oid(q, io), q->q_id, io->tag,
			  res, (uint32_t)UBLK_IO_COMMIT_AND_FETCH_REQ);
...
ublk_io_get_buffer_cb(struct spdk_iobuf_entry *iobuf, void *buf)
{
	...
	spdk_trace_record(TRACE_UBLK_BUF_WAIT_DONE, OWNER_TYPE_UBLK, 0,
			  ublk_trace_oid(io->q, io), io->q->q_id, io->tag, io->payload_size);
		   ...
		   ublk_get_buf_cb get_buf_cb)
{
	...
	io->get_buf_cb = get_buf_cb;
		spdk_trace_record(TRACE_UBLK_BUF_WAIT_BEGIN, OWNER_TYPE_UBLK, 0,
			  ublk_trace_oid(io->q, io), io->q->q_id, io->tag, io->payload_size);
...
_ublk_submit_bdev_io(struct ublk_queue *q, struct ublk_io *io)
{
	...
	spdk_trace_record(TRACE_UBLK_BDEV_SUBMIT, OWNER_TYPE_UBLK, 0,
			  ublk_trace_oid(q, io), q->q_id, io->tag, (int)ublk_op,
			  iod->start_sector, num_blocks);
...
ublk_submit_bdev_io(struct ublk_queue *q, struct ublk_io *io)
{
	...
	case UBLK_IO_OP_WRITE:
		if (g_ublk_tgt.user_copy) {
			...
			_ 			spdk_trace_record(TRACE_UBLK_REQ_READY, OWNER_TYPE_UBLK, 0,
					  ublk_trace_oid(q, io),
					  q->q_id, io->tag, (int)ublksrv_get_op(io->iod),
					  io->iod->start_sector, (uint32_t)io->iod->nr_sectors,
					  (uint32_t)io->cmd_op);
		     ...
		     struct ublk_io *io, unsigned tag)
{
	...
	/* Single-IO latency helper: commit preparation (per-IO). */
	if (cmd_op == UBLK_IO_COMMIT_AND_FETCH_REQ) {
		spdk_trace_record(TRACE_UBLK_COMMIT_PREP, OWNER_TYPE_UBLK, 0,
				  ublk_trace_oid(q, io), q->q_id, tag, (uint32_t)io->result, (uint32_t)cmd_op);
	}
...
ublk_io_xmit(struct ublk_queue *q)
{
	...
	q->cmd_inflight += count;
		/* Single-IO latency helper: commit submit (batched). */
	spdk_trace_record(TRACE_UBLK_COMMIT_SUBMIT, OWNER_TYPE_UBLK, 0,
			  ((uint64_t)q->q_id << 32), q->q_id, 0, (uint32_t)count);
