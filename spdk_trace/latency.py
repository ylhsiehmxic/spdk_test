...
def main():
    ...
    bdev_starts_by_rawid = defaultdict(list)  # rawid -> list[(ts, obj)]
    bdev_dones_by_rawid  = defaultdict(list)

    # 額外索引：raid_main -> list of bdev rawid candidates (where rawid has "(raid_main)")
    ...
    for row in filtered:
        ...
        if evt in (E_RAID_START, E_RAID_DONE):
            raid_main, raid_paren = parse_id(rawid)  # rawid 多半是 "Rxxxx (iYYYY)" => main=Rxxxx, paren=iYYYY
            if evt == E_RAID_START:
    ...
    # root bdev id（parent_id）是 RAID rawid 的括號內，例如 "R5917 (i3313)" -> parent_id=i3313
    # root 的 bdev start/done rawid 通常就是 "i3313"（不含括號）
    # 所以我們在 bdev_* 結構中需要用 rawid==parent_id 去找
    def find_root_times(parent_id: str, t1: float, t4: float, max_gap: float) -> Tuple[Optional[float], Optional[float], str]:
    ...
    # base bdev：**只在括號內 == raid_main 的 rawid 候選集合裡找**
    def find_base_pair_for_raid(raid_main: str, t1: float, t4: float) -> Tuple[Optional[str], Optional[float], Optional[float], str]:
        candidates = list(bdev_rawids_by_raid.get(raid_main, set()))
        if not candidates:
            return None, None, None, ""

        best = None  # (dur, rawid, t2, t3, obj)
        multi_pairs = 0
            ...
            for (t2, obj2) in starts:
                ...
                for (t3, obj3) in dones:
                    ...
                    if best is None or dur < best[0]:
                        best = (dur, rawid, t2, t3, obj2 or obj3)
                    break

        if multi_pairs > 1:
            # 同一個 raid interval 內找到多個候選 pair（可能是你 trace 開太多/或真的 split）
            warn(f"RAID '{raid_main}': multiple base pairs found inside raid interval (pairs={multi_pairs}). Choose the shortest one.")

        if best is None:
            return None, None, None, ""
        _, rawid, t2, t3, obj = best
        return rawid, t2, t3, obj
    ...
    for raid_main, (t1, raid_rawid_start) in raid_start.items():
        ...
        # base pair (STRICT bind by raid_main via "(Rxxxx)")
        base_rawid, t2, t3, base_obj = find_base_pair_for_raid(raid_main, t1, t4)

        # 你想輸出 base_bdev_id 像 "i1314 (R5917)" 也 OK（rawid）
        row_out = {
            "raid_id": raid_main,
