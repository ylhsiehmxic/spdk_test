...
{
	return ((uint64_t)q->q_id << 32) | (uint64_t)io->tag;
}
		     ...
		     struct ublk_io *io, unsigned tag)
{
	...
	/* Trace: per-IO commit preparation (queued into io_uring SQ for commit+fetch). */
	if (cmd_op == UBLK_IO_COMMIT_AND_FETCH_REQ) {
		spdk_trace_record(TRACE_UBLK_COMMIT_PREP, OWNER_TYPE_UBLK, 0,
				  (((uint64_t)q->q_id << 32) | (uint32_t)tag),
				  q->q_id, (uint32_t)tag, (uint32_t)cmd_op, (uint32_t)io->result);
	}
